...
3.  **Follow the Schema:** Ensure your JSON matches the corresponding schema in `schemas/v2/`.
    - `last_modified`: Update this timestamp (ISO 8601 UTC, e.g., `2026-02-18T12:00:00.000Z`).
...
- **Schema:** [heroes.schema.json](schemas/v2/heroes.schema.json)
- **Key Fields**:
  - `class` (Enum): `Conqueror`, `Duelist`, `Enchanter`.
  - `movement_type` (Enum): `Ground`, `Flying`, `Hover`.
  - `difficulty` (1-3).
  - `abilities` (Object): Must contain `passive`, `primary`, `defense`, `ultimate`.
...
- **Schema**: [titans.schema.json](schemas/v2/titans.schema.json)

#### 5. Consumables (`data/consumables`)

Loot items found in chests.

- **Schema**: [consumables.schema.json](schemas/v2/consumables.schema.json)

#### 6. Upgrades (`data/upgrades`)

RNG Level-up bonus options.

- **Schema**: [upgrades.schema.json](schemas/v2/upgrades.schema.json)
- **Key Fields**:
...
- **`changelog_index.json`** — Pagination manifest. Schema: `schemas/v2/changelog_index.schema.json`
- **`changelog.json`** — Full patch history array. Schema: `schemas/v2/changelog.schema.json`
- **`changelog_latest.json`** — Most recent patch entry (or `null`).
- **`timeline/{id}.json`** — Per-entity stat snapshots. Schema: `schemas/v2/timeline_entry.schema.json`
- **TypeScript Types:** `types/patch-history.d.ts`

> **Internal Files:** `data/patches.json`, `data/queue.json`, and `data/audit.jsonl` are internal files that are processed at build time.
...
3.  **Patch History (Automatic):**
    - The CI pipeline runs `generate_patch.py` to compute diffs from git, then `build_changelogs.py` to build paginated changelogs, and finally `build_api.py` to deploy everything to GitHub Pages statelessly.
    - There are no patch files or timeline snapshots to manually commit to your PRs. The pipeline handles everything for you.
