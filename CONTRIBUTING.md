...
Generated by the management tools at publish time. **Do not edit manually.**
...
> **Internal Files:** `data/patches.json`, `data/queue.json`, and `data/audit.jsonl` are generated by The Grimoire (manager tool) and should not be edited manually.
...
3.  **Patch History (Automatic):**
    - Patch history files (`changelog.json`, `timeline/`, etc.) are **auto-generated by CI** when your PR is merged to `main`. You do **not** need to update them manually.
    - The pipeline runs `generate_patch.py` (which internally calls `build_changelogs.py`) → `build_api.py` to compute diffs, build paginated changelogs, and deploy everything to GitHub Pages.
    - **Manual Warning:** Manual editing of `changelog.json` or `timeline/` files is **strongly discouraged** as it will be overwritten by the next automated build.
