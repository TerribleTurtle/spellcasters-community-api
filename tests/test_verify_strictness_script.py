...
class TestVerifyStrictnessIntegration:
    ...
    def test_evil_missing_registry(self):
        ...
        with (
            patch("verify_strictness.create_registry", side_effect=Exception("Simulated Failure")),
    ...
    def test_evil_missing_hero_schema(self):
        ...
        with (
            patch("verify_strictness.create_registry", return_value=(mock_registry, mock_map)),
            patch("sys.exit") as mock_exit,
        ):
            verify_strictness.main()
            mock_exit.assert_called_once_with(1)
