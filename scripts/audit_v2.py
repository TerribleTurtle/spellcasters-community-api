...
try:
    ...
    from jsonschema import validators, exceptions, ValidationError
except ImportError:
...
def main():
    ...
    for field_key, values in numeric_fields.items():
        if len(values) < 4: continue # Need enough data points
        
        nums = [v[1] for v in values]
        if len(set(nums)) == 1: continue # All same value
        ...
        for entity_id, val in values:
            ...
            if abs(z_score) > 3.0:
                ...
                outliers.append(f"Schema `{schema_name}`: Field `{field_name}` has value `{val}` in `{entity_id}` (Z-Score: {z_score:.2f}, Mean: {mean:.2f})")
