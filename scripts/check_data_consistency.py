...
def check_item(v1_item, v2_item, category, item_id, path_prefix=""):
    ...
    for key, v1_val in v1_item.items():
        if key in ["$schema", "last_modified", "changelog", "image_required"]: continue
    ...
    if isinstance(v1, dict):
        for k, v in v1.items():
            if k not in v2:
                 ...
                 return False
            if not compare_complex(v, v2[k], f"{path}/{k}"): return False
        return True
    
    if isinstance(v1, list):
        if len(v1) != len(v2): return False
        for i in range(len(v1)):
            if not compare_complex(v1[i], v2[i], f"{path}[{i}]"): return False
        return True
...
def main():
    ...
    for v1_cat, v2_dir in MAPPINGS.items():
        if v1_cat not in v1_data: continue
        items = v1_data[v1_cat]
        
        for item in items:
            if 'spellcaster_id' in item: item_id = item['spellcaster_id']
            elif 'entity_id' in item: item_id = item['entity_id']
            else: continue
